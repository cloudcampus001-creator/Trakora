  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-6 md:space-y-10">
      ...
      <div className="bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-slate-100">
        <h3 className="text-[10px] md:text-xs font-black uppercase tracking-wider text-slate-400 mb-3">Direct Matricule Ledger Search</h3>
        <div className="flex flex-col sm:flex-row gap-2">
          <input className="border-2 border-slate-100 p-3 flex-1 rounded-xl focus:border-slate-400 transition outline-none font-bold text-sm text-slate-900" placeholder="e.g. MAT-001" value={searchMatricule} onChange={e => setSearchMatricule(e.target.value)} />
          <button onClick={searchStudentByMatricule} className="bg-slate-900 text-white p-3 md:px-6 md:py-3 rounded-xl font-bold hover:bg-slate-800 transition shadow-lg shadow-slate-900/20 text-sm">Search Network</button>
        </div>

        {searchedStudent && (
          <form onSubmit={(e) => executeDigitizedPayment(e, searchedStudent, searchAmount, searchType, true)} className="space-y-4 border-t border-slate-100 mt-5 pt-5 md:mt-6 md:pt-6 animate-fadeIn">
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              <div>
                <p className="font-black text-slate-900 text-base md:text-lg flex items-center gap-2"><ShieldCheck className="text-emerald-500 w-4 h-4 md:w-5 md:h-5"/> {searchedStudent.full_name}</p>
                <p className="text-xs md:text-sm text-slate-600 font-medium">Class Group: {searchedStudent.classes?.name || 'Unassigned'}</p>
              </div>
              <span className="text-[9px] md:text-[10px] font-black uppercase tracking-wider bg-slate-200 px-3 py-1.5 rounded-lg text-slate-700">Status: {searchedStudent.application_status}</span>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
              <div>
                <label className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-wider block mb-1.5 md:mb-2">Fee Allocation Type</label>
                <select className="border-2 border-slate-100 p-3 w-full rounded-xl text-slate-900 bg-white font-bold outline-none focus:border-emerald-500 transition text-sm" value={searchType} onChange={e => setSearchType(e.target.value)}>
                  <option value="TUITION">Tuition Allocation</option>
                  <option value="REGISTRATION">Registration Validation</option>
                </select>
              </div>
              <div>
                <label className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-wider block mb-1.5 md:mb-2">Amount Collected (XAF)</label>
                <input type="number" className="border-2 border-slate-100 p-3 w-full rounded-xl text-slate-900 bg-white font-bold outline-none focus:border-emerald-500 transition text-sm" placeholder="e.g. 50000" value={searchAmount} onChange={e => setSearchAmount(e.target.value)} required />
              </div>
            </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-amber-200 overflow-hidden">
        <div className="bg-amber-50 px-4 md:px-6 py-4 md:py-5 border-b border-amber-200 flex justify-between items-center">
          <h2 className="text-base md:text-lg font-black text-amber-900 uppercase tracking-tight truncate mr-2">1. Pending Approvals</h2>
          <span className="bg-amber-200 text-amber-900 font-black text-[10px] md:text-xs px-2 md:px-3 py-1 rounded-lg shadow-sm whitespace-nowrap">{pendingApps.length} Required</span>
        </div>
              ...
              <div key={app.id} className="py-4 md:py-5 first:pt-0 last:pb-0 flex flex-col md:flex-row justify-between md:items-center gap-3 md:gap-4">
                <div className="w-full">
                  <div className="flex justify-between items-baseline mb-2">
                    ...
                    <p className="text-[9px] md:text-[10px] uppercase font-mono font-black text-slate-400 bg-slate-50 px-2 py-1 rounded border">ID: {app.matricule}</p>
                  </div>
                ...
                <div className="flex flex-row md:flex-col gap-2 w-full md:w-auto mt-2 md:mt-0">
                  <button onClick={() => handleApplicationDecision(app.id, 'APPROVED')} className="flex-1 bg-emerald-600 text-white px-4 py-2.5 rounded-xl text-xs md:text-sm font-bold hover:bg-emerald-700 transition shadow-sm">Accept</button>
                  <button onClick={() => handleApplicationDecision(app.id, 'REJECTED')} className="flex-1 bg-rose-50 text-rose-700 px-4 py-2.5 rounded-xl text-xs md:text-sm font-bold hover:bg-rose-100 transition">Decline</button>
                </div>
              </div>
            ))
          )}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-indigo-200 overflow-hidden">
        <div className="bg-indigo-50 px-4 md:px-6 py-4 md:py-5 border-b border-indigo-200 flex justify-between items-center">
          <h2 className="text-base md:text-lg font-black text-indigo-900 uppercase tracking-tight truncate mr-2">2. Waiting Registration Payment</h2>
          <span className="bg-indigo-200 text-indigo-900 font-black text-[10px] md:text-xs px-2 md:px-3 py-1 rounded-lg shadow-sm whitespace-nowrap">{approvedApps.length} Pending</span>
        </div>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                    <div>
                      ...
                      <div className="text-[10px] md:text-xs font-semibold text-slate-500 mt-1 flex flex-wrap items-center gap-1">
                        ...
                        <span>Fee: <span className={standardFee === 0 ? "text-rose-600 font-black bg-rose-50 px-1.5 md:px-2 py-0.5 rounded" : "text-slate-900 font-black bg-slate-100 px-1.5 md:px-2 py-0.5 rounded"}>{standardFee.toLocaleString()} XAF</span></span>
                      </div>
                    </div>
                    
                    {activePaymentStudent?.id !== app.id ? (
                      <button onClick={() => setActivePaymentStudent(app)} className="w-full sm:w-auto bg-indigo-600 text-white px-4 md:px-5 py-2 md:py-2.5 rounded-xl text-xs font-bold hover:bg-indigo-700 transition shadow-lg shadow-indigo-600/20">Process Cash</button>
                    ) : (
                      <button onClick={() => setActivePaymentStudent(null)} className="w-full sm:w-auto text-xs font-bold text-slate-500 hover:text-slate-700 px-3 py-2 bg-slate-100 rounded-lg">Cancel Form</button>
                    )}
                  </div>

                  {activePaymentStudent?.id === app.id && (
                    <form onSubmit={(e) => executeDigitizedPayment(e, app, standardFee, 'REGISTRATION', false)} className="mt-4 md:mt-5 p-4 md:p-5 bg-slate-50 rounded-xl border border-indigo-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-fadeIn">
                      ...
                      <div className="w-full md:w-auto bg-white px-4 md:px-5 py-2.5 md:py-3 rounded-xl border border-slate-200 text-left md:text-right min-w-[140px] md:min-w-[160px] shadow-sm">
                        ...
                        <p className="font-mono text-lg md:text-xl font-black text-slate-900">{standardFee.toLocaleString()} <span className="text-[10px] md:text-xs text-slate-400">XAF</span></p>
                      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="bg-slate-50 px-4 md:px-6 py-4 md:py-5 border-b border-slate-200 flex justify-between items-center">
          <h2 className="text-base md:text-lg font-black text-slate-800 uppercase tracking-tight truncate mr-2">3. Settlement Ledger</h2>
              ...
              {completedTxns.length === 0 ? (
                ...
                completedTxns.map(tx => (
                  <tr key={tx.id} className="hover:bg-slate-50/80 transition">
                    <td className="p-3 md:p-5 font-black text-slate-900">{tx.students?.full_name || 'System Managed'}</td>
                    <td className="p-3 md:p-5 text-slate-500 font-bold">{tx.students?.classes?.name || 'N/A'}</td>
                    ...
                    <td className="p-3 md:p-5 font-black text-right text-slate-900">{Number(tx.amount).toLocaleString()} <span className="text-[9px] md:text-[10px] text-slate-400">XAF</span></td>
